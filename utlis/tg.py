...
def Ckuser(message):
  ...
  response = requests.get('https://root.ibuser.xyz/apis/join.php?id={}'.format(userID)).json()
  if response["ok"]:
    return True
  elif response["ok"] == False:
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("اضغط للاشتراك ⏺", url="t.me/rrrbe")] ])
